...
class Document(Base):
    ...
    status: Mapped[Document_Status] = mapped_column(default=Document_Status.ACTIVE, index=True)
...
class Processing_Request(Base):
    ...
    document_id: Mapped[int] = mapped_column(Integer, ForeignKey("document.id", ondelete="CASCADE"), nullable=False, index=True)
    processing_type: Mapped[Processing_Type] = mapped_column(default=Processing_Type.DOCUMENT_SUMMARY)
    instructions: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    status: Mapped[Processing_status] = mapped_column(default=Processing_status.PENDING, index=True)
    

    version: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    
    document: Mapped["Document"] = relationship("Document", back_populates="processing_requests")
...
class Extracted_Result(Base):
    ...
    processing_request: Mapped["Processing_Request"] = relationship("Processing_Request", back_populates="extracted_results")
...
class Processing_Job(Base):
    ...
    status: Mapped[Processing_Job_Status] = mapped_column(default=Processing_Job_Status.QUEUED, index=True)

    processing_request: Mapped["Processing_Request"] = relationship("Processing_Request", back_populates="processing_jobs")
